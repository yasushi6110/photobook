...
class PhotoBlock:
    ...
    def update_image(self):
        ...
        if image.width > image.height:
            ...
            new_height = int((PREVIEW_MAX_SIZE / image.width) * image.height)
        else:
            new_height = PREVIEW_MAX_SIZE
            new_width = int((PREVIEW_MAX_SIZE / image.height) * image.width)
        self.preview_img = image.resize((new_width, new_height), Image.LANCZOS)

    def switch_status(self, item):
        # type: (PhotoBlock) -> None
        """ ブロックの状態を別のブロックと入れ替え
        """
...
class PhotoBlockItem(QtWidgets.QGraphicsItem):
    ...
    def __init__(self, block: PhotoBlock, scene_rect: QtCore.QRectF, parent_view):
        super().__init__()
        self.block = block
    ...
    def show_context_menu(self, pos):
        ...
        rotate_action = menu.addAction("90°回転")
        reset_action = menu.addAction("リセット")
        action = menu.exec(QtCore.QPoint(int(pos.x()), int(pos.y())))
        if action == rotate_action:
            self.block.rotation = (self.block.rotation + 90) % 360
            self.update()
        elif action == reset_action:
            self.block.offset_x = 0
            self.block.offset_y = 0
            self.block.scale = 1.0
    ...
    def paint(self, painter, option, widget=None):
        ...
        if render_image:
            ...
            # スケール倍率を計算
            ratio = max(self.rect.width() / iw, self.rect.height() / ih) * self.block.scale
            ...
            # 回転（アスペクト比維持、中心基準）
            img = img.rotate(-self.block.rotation, expand=False, resample=Image.BICUBIC)
...
class PhotoCollageView(QtWidgets.QGraphicsView):
    ...
    def __init__(self, layout_name, long_side_px=3508, portrait=True):
        ...
        self.blocks = []  # type: list[PhotoBlock]
        self.block_count = 1
    ...
    def set_canvas_by_long_side(self, long_side_px, portrait=True):
        self.portrait = portrait
        a4_ratio = math.sqrt(2)
        if portrait:
            self.canvas_height_px = long_side_px
            self.canvas_width_px = int(long_side_px / a4_ratio)
        else:
            self.canvas_width_px = long_side_px
            self.canvas_height_px = int(long_side_px / a4_ratio)
        self.draw_layout()
